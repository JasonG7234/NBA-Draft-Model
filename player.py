...
def draw_conclusions_on_player(df, player_name, by_position=True, to_print=True):
    ...
    get_percentile_rank(df, 'DEF RTG', player_name, to_print=False, is_inverse_percentile=True, to_drop_column=False, rank_col_name="DEF RTG Rank")
    df['Defense Score'] = round((df['STL% Rank']+df['BLK% Rank']+df['DEF RTG Rank'])/3, 1)
    if (to_print): print(f"Defense Score: " + str(get_percentile_rank(df, 'Defense Score', player_name, to_print=False)))
